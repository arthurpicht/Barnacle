...
Simple marker annotation. Marks class to be processed by barnacle.

#### @CloneableVo

The resulting VO implements the `Cloneable` interface and will contain a `clone` method.

No parameters.

#### @SerializableVo

The resulting VO implements the `Serializable` interface and will contain a private constant `serialVersionUID`.
...
In order to make generated VO classes serializable do the following:

* Let the upper VOF class implement the `Serializable` interface
* Implement a `private static final long serialVersionUID` in the VOF class
* Annotate the VOF class with the `SerializableVo` annotation and give an appropriate `serialVersionUID` parameter.
  As a result, the generated VO class will contain a corresponding attribute.
